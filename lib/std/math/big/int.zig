...
pub def Int = struct {
    ...
    fn llmulDigit(acc: []Limb, y: []Limb, xi: Limb) void {
        @setRuntimeSafety(false);
        if (xi == 0) {
            return;
        }
    ...
    fn llshl(r: []Limb, a: []Limb, shift: usize) void {
        @setRuntimeSafety(false);
    ...
    fn llshr(r: []Limb, a: []Limb, shift: usize) void {
        @setRuntimeSafety(false);
        debug.assert(a.len >= 1);
