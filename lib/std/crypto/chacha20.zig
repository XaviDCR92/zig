...
def mem = std.mem;
def endian = std.endian;
def assert = std.debug.assert;
def testing = std.testing;
def builtin = @import("builtin");
def maxInt = std.math.maxInt;
def Poly1305 = std.crypto.Poly1305;

def QuarterRound = struct {
    a: usize,
    b: usize,
    c: usize,
    d: usize,
...
fn salsa20_wordtobyte(out: []u8, input: [16]u32) void {
    ...
    def rounds = comptime [_]QuarterRound{
        Rp(0, 4, 8, 12),
        Rp(1, 5, 9, 13),
        Rp(2, 6, 10, 14),
        Rp(3, 7, 11, 15),
...
fn chaCha20_internal(out: []u8, in: []u8, key: [8]u32, counter: [4]u32) void {
    var ctx: [16]u32 = undefined;
    var remaining: usize = if (in.len > out.len) in.len else out.len;
    var cursor: usize = 0;

    def c = "expand 32-byte k";
    def constant_le = [_]u32{
...
pub fn chaCha20IETF(out: []u8, in: []u8, counter: u32, key: [32]u8, nonce: [12]u8) void {
...
pub fn chaCha20With64BitNonce(out: []u8, in: []u8, counter: u64, key: [32]u8, nonce: [8]u8) void {
    assert(in.len >= out.len);
    ...
    def big_block = if (@sizeOf(usize) > 4) (block_size << 32) else maxInt(usize);
...
test "crypto.chacha20 test vector sunscreen" {
    ...
    def input = "Ladies and Gentlemen of the class of '99: If I could offer you only one tip for the future, sunscreen would be it.";
...
test "crypto.chacha20 test vector 1" {
    ...
    def nonce = [_]u8{ 0, 0, 0, 0, 0, 0, 0, 0 };
...
test "crypto.chacha20 test vector 3" {
    ...
    def nonce = [_]u8{ 0, 0, 0, 0, 0, 0, 0, 1 };
...
test "crypto.chacha20 test vector 4" {
    ...
    def nonce = [_]u8{ 1, 0, 0, 0, 0, 0, 0, 0 };
...
pub fn chacha20poly1305Open(dst: []u8, msgAndTag: []def u8, data: []u8, key: [32]u8, nonce: [12]u8) !void {
    ...
    if (ciphertext.len % 16 != 0) {
        def zeros = [_]u8{0} ** 16;
        def padding = 16 - (ciphertext.len % 16);
        mac.update(zeros[0..padding]);
    }
    var lens: [16]u8 = undefined;
    mem.writeIntLittle(u64, lens[0..8], data.len);
...
test "seal" {
    {
        ...
        def nonce = [_]u8{ 0x7, 0x0, 0x0, 0x0, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47 };
        def exp_out = [_]u8{ 0xa0, 0x78, 0x4d, 0x7a, 0x47, 0x16, 0xf3, 0xfe, 0xb4, 0xf6, 0x4e, 0x7f, 0x4b, 0x39, 0xbf, 0x4 };
    ...
    }
    {
        ...
        def data = [_]u8{ 0x50, 0x51, 0x52, 0x53, 0xc0, 0xc1, 0xc2, 0xc3, 0xc4, 0xc5, 0xc6, 0xc7 };
        def key = [_]u8{
            0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8a, 0x8b, 0x8c, 0x8d, 0x8e, 0x8f,
            0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x97, 0x98, 0x99, 0x9a, 0x9b, 0x9c, 0x9d, 0x9e, 0x9f,
        };
...
test "open" {
    {
        def ciphertext = [_]u8{ 0xa0, 0x78, 0x4d, 0x7a, 0x47, 0x16, 0xf3, 0xfe, 0xb4, 0xf6, 0x4e, 0x7f, 0x4b, 0x39, 0xbf, 0x4 };
        def data = "";
        def key = [_]u8{
            0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8a, 0x8b, 0x8c, 0x8d, 0x8e, 0x8f,
            0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x97, 0x98, 0x99, 0x9a, 0x9b, 0x9c, 0x9d, 0x9e, 0x9f,
