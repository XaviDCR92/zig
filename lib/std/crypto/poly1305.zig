...
pub def Poly1305 = struct {
    def Self = @This();

    pub def mac_length = 16;
    ...
    fn polyBlock(ctx: *Self) void {
        ...
        def s2 = @as(u64, ctx.h[2]) + ctx.c[2]; // s2 <= 1_fffffffe
        def s3 = @as(u64, ctx.h[3]) + ctx.c[3]; // s3 <= 1_fffffffe
        def s4 = @as(u64, ctx.h[4]) + ctx.c[4]; // s4 <=          5

        // Local all the things!
        def r0 = ctx.r[0]; // r0  <= 0fffffff
        def r1 = ctx.r[1]; // r1  <= 0ffffffc
        def r2 = ctx.r[2]; // r2  <= 0ffffffc
        def r3 = ctx.r[3]; // r3  <= 0ffffffc
        def rr0 = (r0 >> 2) * 5; // rr0 <= 13fffffb // lose 2 bits...
        def rr1 = (r1 >> 2) + r1; // rr1 <= 13fffffb // rr1 == (r1 >> 2) * 5
        def rr2 = (r2 >> 2) + r2; // rr2 <= 13fffffb // rr1 == (r2 >> 2) * 5
        def rr3 = (r3 >> 2) + r3; // rr3 <= 13fffffb // rr1 == (r3 >> 2) * 5

        // (h + c) * r, without carry propagation
        def x0 = s0 * r0 + s1 * rr3 + s2 * rr2 + s3 * rr1 + s4 * rr0; //<=97ffffe007fffff8
        def x1 = s0 * r1 + s1 * r0 + s2 * rr3 + s3 * rr2 + s4 * rr1; //<=8fffffe20ffffff6
        def x2 = s0 * r2 + s1 * r1 + s2 * r0 + s3 * rr3 + s4 * rr2; //<=87ffffe417fffff4
        def x3 = s0 * r3 + s1 * r2 + s2 * r1 + s3 * r0 + s4 * rr3; //<=7fffffe61ffffff2
        def x4 = s4 * (r0 & 3); // ...recover 2 bits      //<=               f

        // partial reduction modulo 2^130 - 5
        def _u5 = @truncate(u32, x4 + (x3 >> 32)); // u5 <= 7ffffff5
        def _u0 = (_u5 >> 2) * 5 + (x0 & 0xffffffff);
        def _u1 = (_u0 >> 32) + (x1 & 0xffffffff) + (x0 >> 32);
        def _u2 = (_u1 >> 32) + (x2 & 0xffffffff) + (x1 >> 32);
        def _u3 = (_u2 >> 32) + (x3 & 0xffffffff) + (x2 >> 32);
        def _u4 = (_u3 >> 32) + (_u5 & 3);
    ...
    pub fn final(ctx: *Self, out: []u8) void {
        ...
        def _u0 = @as(u64, 5) + ctx.h[0]; // <= 1_00000004
        def _u1 = (_u0 >> 32) + ctx.h[1]; // <= 1_00000000
        def _u2 = (_u1 >> 32) + ctx.h[2]; // <= 1_00000000
        def _u3 = (_u2 >> 32) + ctx.h[3]; // <= 1_00000000
        def _u4 = (_u3 >> 32) + ctx.h[4]; // <=          5
        // u4 indicates how many times we should subtract 2^130-5 (0 or 1)

        // h + pad, minus 2^130-5 if u4 exceeds 3
        def uu0 = (_u4 >> 2) * 5 + ctx.h[0] + ctx.pad[0]; // <= 2_00000003
        def uu1 = (uu0 >> 32) + ctx.h[1] + ctx.pad[1]; // <= 2_00000000
        def uu2 = (uu1 >> 32) + ctx.h[2] + ctx.pad[2]; // <= 2_00000000
        def uu3 = (uu2 >> 32) + ctx.h[3] + ctx.pad[3]; // <= 2_00000000
...
test "poly1305 rfc7439 vector1" {
    def expected_mac = "\xa8\x06\x1d\xc1\x30\x51\x36\xc6\xc2\x2b\x8b\xaf\x0c\x01\x27\xa9";

    def msg = "Cryptographic Forum Research Group";
    def key = "\x85\xd6\xbe\x78\x57\x55\x6d\x33\x7f\x44\x52\xfe\x42\xd5\x06\xa8" ++
