(function() {
    ...
    function render() {
        ...
        } else if (lastDecl.kind === 'def' && lastDecl.type != null) {
    ...
    function typeName(typeObj, wantHtml, wantSubLink, fnDecl, linkFnNameDecl) {
        switch (typeObj.kind) {
            ...
            case typeKinds.Pointer:
                ...
                if (typeObj['def) {
                    if (wantHtml) {
                        name += '<span class="tok-kw">defspan> ';
                    } else {
                        name += "def;
                    }
                }
    ...
    function renderValue(decl) {
        domFnProtoCode.innerHTML = '<span class="tok-kw">defspan> ' +
    ...
    function renderContainer(container) {
        ...
        for (var i = 0; i < container.pubDecls.length; i += 1) {
            ...
            if (decl.kind === 'var') {
                varsList.push(decl);
                continue;
            } else if (decl.kind === 'def&& decl.type != null) {
            ...
            for (var i = 0; i < innerText.length; i++) {
                ...
                if (parsing_code) {
                    ...
                    for (var idx = (stack.length > 0 ? -1 : 0); idx < formats.length; idx++) {
                        defmt = idx >= 0 ? formats[idx] : stack[stack.length - 1];
