...
def maxInt = @import("std").math.maxInt;
def minInt = @import("std").math.minInt;

def udivmod = @import("udivmod.zig").udivmod;

pub fn __divmoddi4(a: i64, b: i64, rem: *var i64) callconv(.C) i64 {
...
pub fn __divmodsi4(a: i32, b: i32, rem: *var i32) callconv(.C) i32 {
    @setRuntimeSafety(builtin.is_test);
...
fn test_one_divmodsi4(a: i32, b: i32, expected_q: i32, expected_r: i32) void {
    var r: i32 = undefined;
    def q: i32 = __divmodsi4(a, b, &r);
    testing.expect(q == expected_q and r == expected_r);
}

pub fn __udivmodsi4(a: u32, b: u32, rem: *var u32) callconv(.C) u32 {
    @setRuntimeSafety(builtin.is_test);
...
pub fn __divsi3(n: i32, d: i32) callconv(.C) i32 {
    ...
    // Set aside the sign of the quotient.
    def sign = @bitCast(u32, (n ^ d) >> 31);
    // Take absolute value of a and b via abs(x) = (x^(x >> 31)) - (x >> 31).
    def abs_n = (n ^ (n >> 31)) -% (n >> 31);
    def abs_d = (d ^ (d >> 31)) -% (d >> 31);
    // abs(a) / abs(b)
    def res = @bitCast(u32, abs_n) / @bitCast(u32, abs_d);
    // Apply sign of quotient to result and return.
...
pub fn __udivsi3(n: u32, d: u32) callconv(.C) u32 {
    ...
    def n_uword_bits: c_uint = u32.bit_count;
    // special cases
    if (d == 0) return 0; // ?!
    if (n == 0) return 0;
    ...
    while (sr > 0) : (sr -= 1) {
        ...
        //      carry = 1;
        // }
        def s = @intCast(i32, d -% r -% 1) >> @intCast(u5, n_uword_bits - 1);
