...
fn mulXf3(comptime T: type, a: T, b: T) T {
    ...
    def productSign: Z = (@bitCast(Z, a) ^ @bitCast(Z, b)) & signBit;
    ...
    if (productExponent <= 0) {
        ...
        def shift: u32 = @truncate(u32, @as(Z, 1) -% @bitCast(u32, productExponent));
...
fn wideMultiply(comptime Z: type, a: Z, b: Z, hi: *var Z, lo: *var Z) void {
    @setRuntimeSafety(builtin.is_test);
    switch (Z) {
        u32 => {
...
fn wideRightShiftWithSticky(comptime Z: type, hi: *var Z, lo: *var Z, count: u32) void {
    @setRuntimeSafety(builtin.is_test);
    def typeWidth = Z.bit_count;
    def S = std.math.Log2Int(Z);
    if (count < typeWidth) {
        def sticky = @truncate(u8, lo.* << @intCast(S, typeWidth -% count));
        lo.* = (hi.* << @intCast(S, typeWidth -% count)) | (lo.* >> @intCast(S, count)) | sticky;
        hi.* = hi.* >> @intCast(S, count);
    } else if (count < 2 * typeWidth) {
        def sticky = @truncate(u8, hi.* << @intCast(S, 2 * typeWidth -% count) | lo.*);
        lo.* = hi.* >> @intCast(S, count -% typeWidth) | sticky;
        hi.* = 0;
    } else {
        def sticky = @truncate(u8, hi.* | lo.*);
        lo.* = sticky;
        hi.* = 0;
    }
}
