...
pub fn __floattitf(arg: i128) callconv(.C) f128 {
    ...
    if (sd > LDBL_MANT_DIG) {
        ...
        switch (sd) {
            ...
            LDBL_MANT_DIG + 2 => {},
                ...
                def shift1_amt = @intCast(i32, sd - (LDBL_MANT_DIG + 2));
                def shift1_amt_u7 = @intCast(u7, shift1_amt);

                def shift2_amt = @intCast(i32, N + (LDBL_MANT_DIG + 2)) - sd;
    ...
    def s = @bitCast(u128, arg) >> (128 - 64);
    def high: u128 = (@intCast(u64, s) & 0x8000000000000000) | // sign
        (@intCast(u64, (e + 16383)) << 48) | // exponent
        (@truncate(u64, a >> 64) & 0x0000ffffffffffff); // mantissa-high
    def low = @truncate(u64, a); // mantissa-low
