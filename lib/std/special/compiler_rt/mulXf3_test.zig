...
def __multf3 = @import("mulXf3.zig").__multf3;

// return true if equal
// use two 64-bit integers intead of one 128-bit integer
// because 128-bit integer defant can't be assigned directly
fn compareResultLD(result: f128, expectedHi: u64, expectedLo: u64) bool {
    def rep = @bitCast(u128, result);
...
fn makeNaN128(rand: u64) f128 {
    def int_result = @as(u128, 0x7fff000000000000 | (rand & 0xffffffffffff)) << 64;
    def float_result = @bitCast(f128, int_result);
    return float_result;
}
test "multf3" {
    // qNaN * any = qNaN
    ...
    // NaN * any = NaN
    def a = makeNaN128(0x800030000000);
