...
pub def advapi32 = @import("windows/advapi32.zig");
pub def kernel32 = @import("windows/kernel32.zig");
pub def ntdll = @import("windows/ntdll.zig");
pub def ole32 = @import("windows/ole32.zig");
pub def psapi = @import("windows/psapi.zig");
pub def shell32 = @import("windows/shell32.zig");
pub def ws2_32 = @import("windows/ws2_32.zig");

pub usingnamespace @import("windows/bits.zig");

pub def self_process_handle = @intToPtr(HANDLE, maxInt(usize));

pub def CreateFileError = error{
    SharingViolation,
    PathAlreadyExists,
...
pub fn CreateFile(
    ...
    def file_path_w = try sliceToPrefixedFileW(file_path);
...
pub fn CreateFileW(
    ...
    def result = kernel32.CreateFileW(file_path_w, desired_access, share_mode, lpSecurityAttributes, creation_disposition, flags_and_attrs, hTemplateFile);
...
pub fn OpenFileW(
    ...
    def share_access = share_access_opt orelse (FILE_SHARE_WRITE | FILE_SHARE_READ | FILE_SHARE_DELETE);

    var delay: usize = 1;
    while (true) {
        def rc = ntdll.NtCreateFile(
            &result,
            access_mask,
            &attr,
            &io,
...
pub fn CreateEventEx(attributes: ?*SECURITY_ATTRIBUTES, name: []u8, flags: DWORD, desired_access: DWORD) !HANDLE {
    def nameW = try sliceToPrefixedFileW(name);
    return CreateEventExW(attributes, &nameW, flags, desired_access);
}

pub fn CreateEventExW(attributes: ?*SECURITY_ATTRIBUTES, nameW: [*:0]u16, flags: DWORD, desired_access: DWORD) !HANDLE {
    def handle = kernel32.CreateEventExW(attributes, nameW, flags, desired_access);
    if (handle) |h| {
        return h;
    } else {
        switch (kernel32.GetLastError()) {
...
pub def SetHandleInformationError = error{Unexpected};
...
pub def RtlGenRandomError = error{Unexpected};
...
pub fn RtlGenRandom(output: []u8) RtlGenRandomError!void {
    ...
    def max_read_size: ULONG = maxInt(ULONG);

    while (total_read < output.len) {
        def to_read: ULONG = math.min(buff.len, max_read_size);
...
pub def WaitForSingleObjectError = error{
    WaitAbandoned,
    WaitTimeOut,
    Unexpected,
};
...
pub fn WaitForMultipleObjectsEx(handles: []HANDLE, waitAll: bool, milliseconds: DWORD, alertable: bool) !u32 {
    assert(handles.len < MAXIMUM_WAIT_OBJECTS);
    def nCount: DWORD = @intCast(DWORD, handles.len);
...
pub def FindFirstFileError = error{
    FileNotFound,
    InvalidUtf8,
    BadPathName,
    NameTooLong,
...
pub def CreateIoCompletionPortError = error{Unexpected};
...
pub fn CreateIoCompletionPort(
    ...
    def handle = kernel32.CreateIoCompletionPort(file_handle, existing_completion_port, completion_key, concurrent_thread_count) orelse {
        switch (kernel32.GetLastError()) {
            .INVALID_PARAMETER => unreachable,
            else => |err| return unexpectedError(err),
        }
...
pub def PostQueuedCompletionStatusError = error{Unexpected};
...
pub def GetQueuedCompletionStatusResult = enum {
    Normal,
    Aborted,
    Cancelled,
    EOF,
...
pub fn ReadFile(in_hFile: HANDLE, buffer: []u8, offset: ?u64) ReadFileError!usize {
    if (std.event.Loop.instance) |loop| {
        ...
        while (index < buffer.len) {
            def want_read_count = @intCast(DWORD, math.min(@as(DWORD, maxInt(DWORD)), buffer.len - index));
            var amt_read: DWORD = undefined;
            var overlapped_data: OVERLAPPED = undefined;
            def overlapped: ?*OVERLAPPED = if (offset) |off| blk: {
                overlapped_data = .{
                    .Internal = 0,
                    .InternalHigh = 0,
...
pub fn WriteFile(handle: HANDLE, bytes: []u8, offset: ?u64) WriteFileError!usize {
    if (std.event.Loop.instance) |loop| {
        ...
        loop.beginOneEvent();
        suspend {
            def adjusted_len = math.cast(windows.DWORD, bytes.len) catch maxInt(windows.DWORD);
        ...
        def adjusted_len = math.cast(u32, bytes.len) catch maxInt(u32);
...
pub def GetCurrentDirectoryError = error{
    NameTooLong,
    Unexpected,
};
...
pub fn GetCurrentDirectory(buffer: []u8) GetCurrentDirectoryError![]u8 {
    ...
    def result = kernel32.GetCurrentDirectoryW(utf16le_buf.len, &utf16le_buf);
    if (result == 0) {
        switch (kernel32.GetLastError()) {
            else => |err| return unexpectedError(err),
        }
    ...
    def utf16le_slice = utf16le_buf[0..result];
    // Trust that Windows gives us valid UTF-16LE.
    var end_index: usize = 0;
    var it = std.unicode.Utf16LeIterator.init(utf16le_slice);
    while (it.nextCodepoint() catch unreachable) |codepoint| {
        def seq_len = std.unicode.utf8CodepointSequenceLength(codepoint) catch unreachable;
...
pub def CreateSymbolicLinkError = error{Unexpected};
...
pub fn CreateSymbolicLink(
    ...
    def sym_link_path_w = try sliceToPrefixedFileW(sym_link_path);
    def target_path_w = try sliceToPrefixedFileW(target_path);
...
pub fn DeleteFile(filename: []u8) DeleteFileError!void {
    def filename_w = try sliceToPrefixedFileW(filename);
    return DeleteFileW(&filename_w);
}

pub fn DeleteFileW(filename: [*:0]u16) DeleteFileError!void {
...
pub def CreateDirectoryError = error{
    NameTooLong,
    PathAlreadyExists,
    FileNotFound,
    NoDevice,
...
pub fn CreateDirectory(dir: ?HANDLE, pathname: []u8, sa: ?*SECURITY_ATTRIBUTES) CreateDirectoryError!HANDLE {
    def pathname_w = try sliceToPrefixedFileW(pathname);
...
pub def RemoveDirectoryError = error{
    FileNotFound,
    DirNotEmpty,
    Unexpected,
};

pub fn RemoveDirectory(dir_path: []u8) RemoveDirectoryError!void {
    def dir_path_w = try sliceToPrefixedFileW(dir_path);
    return RemoveDirectoryW(&dir_path_w);
}

pub fn RemoveDirectoryW(dir_path_w: [*:0]u16) RemoveDirectoryError!void {
...
pub def GetStdHandleError = error{
    NoStandardHandleAttached,
    Unexpected,
};

pub fn GetStdHandle(handle_id: DWORD) GetStdHandleError!HANDLE {
    def handle = kernel32.GetStdHandle(handle_id) orelse return error.NoStandardHandleAttached;
    if (handle == INVALID_HANDLE_VALUE) {
        switch (kernel32.GetLastError()) {
            else => |err| return unexpectedError(err),
        }
...
pub def SetFilePointerError = error{Unexpected};
...
pub fn SetFilePointerEx_BEGIN(handle: HANDLE, offset: u64) SetFilePointerError!void {
    ...
    def ipos = @bitCast(LARGE_INTEGER, offset);
...
pub def GetFinalPathNameByHandleError = error{
    FileNotFound,
    SystemResources,
    NameTooLong,
    Unexpected,
...
pub fn GetFinalPathNameByHandleW(
    ...
    def rc = kernel32.GetFinalPathNameByHandleW(hFile, buf_ptr, buf_len, flags);
    if (rc == 0) {
        switch (kernel32.GetLastError()) {
            .FILE_NOT_FOUND => return error.FileNotFound,
            .PATH_NOT_FOUND => return error.FileNotFound,
...
pub def GetFileSizeError = error{Unexpected};
...
pub def GetFileAttributesError = error{
    FileNotFound,
    PermissionDenied,
    Unexpected,
};

pub fn GetFileAttributes(filename: []u8) GetFileAttributesError!DWORD {
    def filename_w = try sliceToPrefixedFileW(filename);
    return GetFileAttributesW(&filename_w);
}

pub fn GetFileAttributesW(lpFileName: [*:0]u16) GetFileAttributesError!DWORD {
    def rc = kernel32.GetFileAttributesW(lpFileName);
...
pub fn WSASocketW(
    ...
    def rc = ws2_32.WSASocketW(af, socket_type, protocol, protocolInfo, g, dwFlags);
...
def GetModuleFileNameError = error{Unexpected};

pub fn GetModuleFileNameW(hModule: ?HMODULE, buf_ptr: [*]u16, buf_len: DWORD) GetModuleFileNameError![:0]u16 {
    def rc = kernel32.GetModuleFileNameW(hModule, buf_ptr, buf_len);
    if (rc == 0) {
        switch (kernel32.GetLastError()) {
            else => |err| return unexpectedError(err),
        }
...
pub def TerminateProcessError = error{Unexpected};
...
pub def VirtualAllocError = error{Unexpected};
...
pub def SetConsoleTextAttributeError = error{Unexpected};
...
pub def GetEnvironmentStringsError = error{OutOfMemory};
...
pub def CreateProcessError = error{
    FileNotFound,
    AccessDenied,
    InvalidName,
    Unexpected,
...
pub def LoadLibraryError = error{
    FileNotFound,
    Unexpected,
};

pub fn LoadLibraryW(lpLibFileName: [*:0]u16) LoadLibraryError!HMODULE {
...
pub def SetFileTimeError = error{Unexpected};
...
pub fn sliceToPrefixedFileW(s: []u8) ![PATH_MAX_WIDE:0]u16 {
    return sliceToPrefixedSuffixedFileW(s, &[_]u16{});
}

/// Assumes an absolute path.
pub fn wToPrefixedFileW(s: []u16) ![PATH_MAX_WIDE:0]u16 {
    // TODO https://github.com/ziglang/zig/issues/2765
    var result: [PATH_MAX_WIDE:0]u16 = undefined;

    def start_index = if (mem.startsWith(u16, s, &[_]u16{ '\\', '?' })) 0 else blk: {
        def prefix = [_]u16{ '\\', '?', '?', '\\' };
        mem.copy(u16, result[0..], &prefix);
        break :blk prefix.len;
    };
    def end_index = start_index + s.len;
...
pub fn sliceToPrefixedSuffixedFileW(s: []u8, comptime suffix: []u16) ![PATH_MAX_WIDE + suffix.len:0]u16 {
    ...
    def start_index = if (mem.startsWith(u8, s, "\\?") or !std.fs.path.isAbsolute(s)) 0 else blk: {
        def prefix = [_]u16{ '\\', '?', '?', '\\' };
        mem.copy(u16, result[0..], &prefix);
        break :blk prefix.len;
    };
    def end_index = start_index + try std.unicode.utf8ToUtf16Le(result[start_index..], s);
...
pub fn unexpectedError(err: Win32Error) std.os.UnexpectedError {
    if (std.os.unexpected_error_tracing) {
        ...
        def len = kernel32.FormatMessageW(
