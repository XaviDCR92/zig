...
pub def BootServices = extern struct {
    hdr: TableHeader,
    ...
    /// Creates an event.
    createEvent: extern fn (u32, usize, ?extern fn (Event, ?*c_void) void, ?*c_void, *Event) Status,
    ...
    handleProtocol: extern fn (Handle, *align(8) def Guid, *?*c_void) Status,
    ...
    locateHandle: extern fn (LocateSearchType, ?*align(8) def Guid, ?*c_void, *usize, [*]Handle) Status,

    locateDevicePath: Status, // TODO
    installConfigurationTable: Status, // TODO

    /// Loads an EFI image into memory.
    loadImage: extern fn (bool, Handle, ?*DevicePathProtocol, ?[*]u8, usize, *?Handle) Status,

    /// Transfers control to a loaded image's entry point.
    startImage: extern fn (Handle, ?*usize, ?*[*]u16) Status,

    /// Terminates a loaded EFI image and returns control to boot services.
    exit: extern fn (Handle, Status, usize, ?*c_void) Status,
    ...
    /// Queries a handle to determine if it supports a specified protocol.
    openProtocol: extern fn (Handle, *align(8) def Guid, *?*c_void, ?Handle, ?Handle, OpenProtocolAttributes) Status,

    /// Closes a protocol on a handle that was opened using openProtocol().
    closeProtocol: extern fn (Handle, *align(8) def Guid, Handle, ?Handle) Status,

    /// Retrieves the list of agents that currently have a protocol interface opened.
    openProtocolInformation: extern fn (Handle, *align(8) def Guid, *[*]ProtocolInformationEntry, *usize) Status,

    /// Retrieves the list of protocol interface GUIDs that are installed on a handle in a buffer allocated from pool.
    protocolsPerHandle: extern fn (Handle, *[*]*align(8) def Guid, *usize) Status,

    /// Returns an array of handles that support the requested protocol in a buffer allocated from pool.
    locateHandleBuffer: extern fn (LocateSearchType, ?*align(8) def Guid, ?*c_void, *usize, *[*]Handle) Status,

    /// Returns the first protocol instance that matches the given protocol.
    locateProtocol: extern fn (*align(8) def Guid, ?*c_void, *?*c_void) Status,
    ...
    pub def signature: u64 = 0x56524553544f4f42;

    pub def event_timer: u32 = 0x80000000;
    pub def event_runtime: u32 = 0x40000000;
    pub def event_notify_wait: u32 = 0x00000100;
    pub def event_notify_signal: u32 = 0x00000200;
    pub def event_signal_exit_boot_services: u32 = 0x00000201;
    pub def event_signal_virtual_address_change: u32 = 0x00000202;

    pub def tpl_application: usize = 4;
    pub def tpl_callback: usize = 8;
    pub def tpl_notify: usize = 16;
    pub def tpl_high_level: usize = 31;
};

pub def TimerDelay = extern enum(u32) {
    TimerCancel,
    TimerPeriodic,
    TimerRelative,
};

pub def MemoryType = extern enum(u32) {
    ReservedMemoryType,
    LoaderCode,
    LoaderData,
    BootServicesCode,
...
pub def MemoryDescriptor = extern struct {
    type: MemoryType,
    physical_start: u64,
    virtual_start: u64,
...
pub def LocateSearchType = extern enum(u32) {
    AllHandles,
    ByRegisterNotify,
    ByProtocol,
};

pub def OpenProtocolAttributes = packed struct {
...
pub def ProtocolInformationEntry = extern struct {
    agent_handle: ?Handle,
...
pub def AllocateType = extern enum(u32) {
    AllocateAnyPages,
    AllocateMaxAddress,
    AllocateAddress,
};
