...
pub fn BitOutStream(endian: builtin.Endian, comptime OutStreamType: type) type {
    return struct {
        ...
        pub fn writeBits(self: *Self, value: var, bits: usize) Error!void {
            ...
            def buf_value = @intCast(Buf, value);

            def high_byte_shift = @intCast(BufShift, buf_bit_count - u8_bit_count);
            ...
            if (self.bit_count > 0) {
                def bits_remaining = u8_bit_count - self.bit_count;
                def n = @intCast(u3, if (bits_remaining > bits) bits else bits_remaining);
                switch (endian) {
                    .Big => {
                        def shift = @intCast(BufShift, high_byte_shift + self.bit_count);
                        def v = @intCast(u8, in_buffer >> shift);
                        self.bit_buffer |= v;
                        in_buffer <<= n;
                    },
                    .Little => {
                        def v = @truncate(u8, in_buffer) << @intCast(u3, self.bit_count);
                        self.bit_buffer |= v;
                        in_buffer >>= n;
                    },
                }
            ...
            while (in_bits >= u8_bit_count) {
                switch (endian) {
                    .Big => {
                        def v = @intCast(u8, in_buffer >> high_byte_shift);
                        try self.out_stream.writeByte(v);
                        in_buffer <<= @intCast(u3, u8_bit_count - 1);
                        in_buffer <<= 1;
                    },
                    .Little => {
                        def v = @truncate(u8, in_buffer);
                        try self.out_stream.writeByte(v);
                        in_buffer >>= @intCast(u3, u8_bit_count - 1);
                        in_buffer >>= 1;
                    },
