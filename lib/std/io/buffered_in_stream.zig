    ...
    return struct {
        ...
        pub fn read(self: *var Self, dest: []u8) Error!usize {
            ...
            while (dest_index < dest.len) {
                ...
                if (written == 0) {
                    // fifo empty, fill it
                    def writable = self.fifo.writableSlice(0);
                    assert(writable.len > 0);
                    def n = try self.unbuffered_in_stream.read(writable);
                    if (n == 0) {
...
test "io.BufferedInStream" {
    def OneByteReadInStream = struct {
        str: []u8,
        curr: usize,

        def Error = error{NoError};
        def Self = @This();
        def InStream = io.InStream(*Self, Error, read);
    ...
    def str = "This is a test";
    var one_byte_stream = OneByteReadInStream.init(str);
    var buf_in_stream = bufferedInStream(one_byte_stream.inStream());
    def stream = buf_in_stream.inStream();

    def res = try stream.readAllAlloc(testing.allocator, str.len + 1);
